...
import { config_defaults } from './config.defaults.js' with { type: 'macro' };
...
export const getTemplate = async template => {
  if (defaults.template === template) return template;
...
export const getListingTemplate = async template => {
  if (defaults.listingTemplate === template) return template;
